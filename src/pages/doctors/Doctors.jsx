...
import GenericModal from "../../api/GenericModals¡";
...
const Doctors = () => {
  ...
  const [ recordedDoctorInfo,setRecordedDoctorInfo ]=useState(null)
  //Post data to API
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setRecordedDoctorInfo(recordDoctorToApi)
      setIsOpen(true)
      reset();
    } catch (error) {
      console.error(error);
  ...
  return (
    <section className="flex justify-around gap-4">
     <GenericModal isOpen={isOpen} setIsOpen={setIsOpen} recordedDoctorInfo={recordedDoctorInfo}/>
