      ...
      <section className="py-6 bg-medBlue">
        <div className="container mx-auto flex flex-col justify-center p-4 space-y-8 md:p-10 lg:space-y-0 lg:space-x-12 lg:justify-between lg:flex-row">
          <div className="flex flex-col space-y-4 text-center lg:text-left">
            <h1 className="text-5xl font-bold leadi text-gray-100">Search for Appointments</h1>
          ...
          <div className="flex flex-row items-center self-center justify-center flex-shrink-0 shadow-md lg:justify-end">
            <form 
              ...
              <input type="date" className="w-3/5 p-3 rounded-l-lg sm:w-2/3"  {...register("fromDate",{required:true})}/>
              <label className="text-gray-100">To</label>
              <input type="date" className="w-3/5 p-3 rounded-l-lg sm:w-2/3"  {...register("toDate",{required:true})}/>
              <button
        ...
        {dataForm.dates ?
        <p className="text-lg text-gray-100">Found appointments from {formatDate(dataForm?.dates?.fromDate)} to {formatDate(dataForm?.dates?.toDate)}</p> :
        <p className="text-lg bg-red-400">{dataForm?.error?.message}</p>}
      </section>
      {appointmentList &&
      <ListTable appointmentList={appointmentList} reloadAppointmentList={reloadAppointmentList}/>
      }
    </div>
  );
};
